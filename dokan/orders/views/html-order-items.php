    ...
    <table cellpadding="0" cellspacing="0" class="woocommerce_order_items dokan-table dokan-table-strip">
        ...
        <tbody id="order_shipping_line_items">
            ...
            foreach ($line_items_shipping as $item_id => $item) {
                dokan_get_template_part(
                    'orders/views/html-order-shipping',
                    '',
                    array(
                        'pro'              => true,
                        'item'             => $item,
        ...
        <tbody id="order_refunds">
            ...
            if ($refunds) {
                foreach ($refunds as $refund) {
                    dokan_get_template_part(
                        'orders/views/html-order-refund',
                        '',
                        array(
                            'pro'          => true,
                            'refund'       => $refund,
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
    ...
    <table class="wc-order-totals">
        ...
        $global_commission_rate = wp_kses_post(dokan()->commission->get_global_rate()) . '%';
        ...
        foreach ($items as $item_id => $item) {
            ...
            // Get the per-product admin commission
            $per_product_admin_commission = get_post_meta($product_id, '_per_product_admin_commission', true);

            // Determine the commission to display
            $commission_to_display = $per_product_admin_commission ? $per_product_admin_commission . '%' : $global_commission_rate;
            ...
            printf(
                '<tr>
                    ...
                    <td>%s</td>
                    <td class="commission_td">%s</td>
                </tr>',
                esc_html($product_name),
                esc_html($commission_to_display)
            );
        }
        ?>
